...
Ngoài các endpoint trong đề bài, nhóm em bổ sung thêm ba endpoint vượt yêu cầu để hệ thống hoàn thiện hơn về mặt vận hành:

\begin{itemize}[nosep]
\item \texttt{GET /summary}: Tổng quan tức thì của toàn hệ thống --- số thùng online, offline, số cần thu gom, số ở mức nguy cấp, và trạng thái rút gọn từng bin. Phù hợp cho màn hình giám sát trung tâm hoặc webhook định kỳ.

\item \texttt{GET /bins/offline}: Sensor offline detection theo yêu cầu nâng cao. So sánh timestamp bản ghi mới nhất với biến môi trường \texttt{SENSOR\_OFFLINE\_TIMEOUT}, trả ra hai danh sách rõ ràng: thùng đang online và thùng đã im lặng quá lâu, kèm thời gian bản ghi cuối cùng nhận được.

\item \texttt{GET /bins/\{id\}/eta}: Dự báo thời điểm thùng đầy theo yêu cầu nâng cao. Lấy chuỗi \texttt{fill\_level} trong mười lăm phút gần nhất, áp hồi quy tuyến tính OLS để tính tốc độ tăng trung bình theo phút, rồi suy ra thời điểm đến mức một trăm phần trăm. Kết quả gồm \texttt{eta\_minutes}, \texttt{eta\_timestamp} (ISO 8601 UTC), \texttt{fill\_rate\_per\_minute}, và mức độ tin cậy (\texttt{high} nếu có từ năm điểm trở lên, \texttt{low} nếu ít hơn). Endpoint trả \texttt{null} khi fill\_level đang giảm hoặc đứng yên, không đưa ra dự báo vô nghĩa. Thuật toán được tách thành module riêng \texttt{gateway\_api/eta\_predictor.py} để kiểm thử độc lập không cần InfluxDB.
\end{itemize}

%-------------------------------------------------------------
\subsection{Thuật toán dự báo thời điểm thùng đầy (OLS Linear Regression)}
%-------------------------------------------------------------

Bài toán đặt ra là: cho $n$ bản đo $(t_i,\,f_i)$ trong mười lăm phút gần nhất, với $t_i$ là Unix timestamp tính theo giây và $f_i$ là \texttt{fill\_level} tính theo phần trăm, hãy ước tính thời điểm $f$ đạt $100\,\%$.

\textbf{Bước 1 — Chuẩn hóa thời gian.} Đặt $x_i = t_i - t_0$ với $t_0 = t_1$ là timestamp của điểm đo đầu tiên. Phép dịch này giữ nguyên hình học của dữ liệu nhưng tránh tràn số khi bình phương (Unix timestamp $\approx 1{,}7 \times 10^9$ giây, bình phương trực tiếp cho $\approx 2{,}9 \times 10^{18}$, gần giới hạn của \texttt{float64}).

\textbf{Bước 2 — Hệ số góc OLS.} Tìm đường thẳng $f = a\,x + b$ tối thiểu hóa tổng bình phương sai số:
\[
a \;=\; \frac{SS_{xy}}{SS_{xx}}, \qquad
SS_{xy} = \sum_{i=1}^{n}(x_i - \bar{x})(f_i - \bar{f}), \qquad
SS_{xx} = \sum_{i=1}^{n}(x_i - \bar{x})^2
\]
Đây là ước lượng \emph{Best Linear Unbiased Estimator} (BLUE) theo định lý Gauss--Markov khi sai số đo không tương quan. Độ phức tạp tính toán là $O(n)$ thời gian và $O(n)$ bộ nhớ, không cần thư viện ngoài.

\textbf{Bước 3 — Phòng vệ chiều.} Nếu $a \leq 0$, thùng đang ổn định hoặc vừa được thu gom nên mức đầy giảm, không có ETA có nghĩa. Endpoint trả \texttt{eta\_minutes: null} kèm ghi chú rõ lý do, thay vì trả một con số âm gây hiểu nhầm.

\textbf{Bước 4 — Tính ETA.} Giải phương trình $f_{\text{last}} + a \cdot \Delta t = 100$:
\[
\Delta t \;=\; \frac{100 - f_{\text{last}}}{a} \quad \text{(giây)}
\]
Thời điểm dự báo $= t_{\text{last}} + \Delta t$, tính từ timestamp \emph{điểm đo cuối cùng} (không phải thời điểm gọi API) vì $f_{\text{last}}$ là giá trị tại điểm đó.

\textbf{Ví dụ số minh họa.} Giả sử năm điểm đo cách nhau $60\,\text{s}$, mức đầy tăng đều từ $46\,\%$ đến $50\,\%$:
...
\[
\bar{x} = 120, \quad \bar{f} = 48{,}06
\]
\[
SS_{xy} = (-120)(-2{,}06) + (-60)(-1{,}06) + 0 + (60)(1{,}04) + (120)(1{,}94) = 606{,}0
\]
\[
SS_{xx} = 120^2 + 60^2 + 0 + 60^2 + 120^2 = 36\,000
\]
\[
a = \frac{606}{36\,000} \approx 0{,}01683\;\%/\text{s} \;\approx\; 1{,}01\;\%/\text{phút}
\]
\[
\Delta t = \frac{100 - 50{,}0}{0{,}01683} \approx 2\,973\;\text{s} \approx 49{,}5\;\text{phút}
\]

Trong điều kiện mô phỏng thực tế, sensor publish mỗi năm đến bảy giây, nên cửa sổ mười lăm phút cung cấp khoảng $128$ đến $180$ điểm đo. Với $n \geq 5$ điểm, endpoint trả \texttt{confidence: "high"}; dưới năm điểm (mới khởi động) trả \texttt{"low"}.

Thuật toán OLS được tách thành hàm thuần \texttt{predict\_eta()} trong module \texttt{gateway\_api/eta\_predictor.py}. Hàm không có I/O, nhận vào danh sách tuple và trả về \texttt{ETAResult}, nên có thể kiểm thử hoàn toàn độc lập không cần Docker hay InfluxDB. Bộ test \texttt{test\_eta\_predictor.py} gồm hai mươi test case bao phủ: dữ liệu hoàn toàn tuyến tính (kiểm tra slope chính xác đến sáu chữ số thập phân), dữ liệu nhiễu, slope âm, chuỗi phẳng, chuỗi một điểm, chuỗi rỗng, thùng đã đầy, và tính bất biến của chuẩn hóa thời gian.
...
Một lệnh duy nhất dựng toàn bộ hệ thống. Mọi tham số cấu hình đọc từ biến môi trường và file \texttt{.env}, theo đúng nguyên tắc mười hai yếu tố (xem Phụ lục A để biết đầy đủ ý nghĩa từng biến). Các service gọi nhau bằng tên service trong mạng \texttt{iot\_net} do Docker Compose tạo ra, tuyệt đối không dùng \texttt{localhost}. Tất cả mười hai service đều có healthcheck: Mosquitto và InfluxDB dùng lệnh kiểm tra tích hợp của chính chúng; Grafana dùng \texttt{wget /api/health} (image Alpine có wget, không có curl); \texttt{gateway-api} dùng \texttt{python3 -c urllib.request.urlopen('/health')} để gọi đúng endpoint FastAPI đã có sẵn; các service không có HTTP port (\texttt{waste-gateway}, sensor, actuator, tb-gateway) dùng \texttt{grep /proc/1/cmdline} để xác nhận tiến trình Python chính còn sống. Mọi service đều đặt chính sách \texttt{restart: unless-stopped}; dữ liệu InfluxDB và Grafana được giữ lại qua named volume kể cả khi container bị xóa và tạo lại. Thành phần ThingsBoard được tách theo profile riêng để chỉ khởi động khi đã có token hợp lệ, tránh lỗi không cần thiết khi chưa cấu hình ThingsBoard.
...
Phạm vi kiểm thử bao gồm rule engine và state store với các trường hợp debounce lệnh, phát hiện cạnh sự kiện và vòng đời thu gom; cầu nối ThingsBoard với việc chuyển đổi RPC, đóng gói payload telemetry và alarm, xử lý shared attributes; module dự báo \texttt{eta\_predictor} với hai mươi test case bao phủ đầy đủ các nhánh OLS (slope dương, âm, bằng không, chuỗi rỗng, thùng đầy, kiểm tra bất biến chuẩn hóa timestamp); REST API với việc kiểm tra đầu vào, mã lỗi 404 và 400, gửi lệnh và đọc InfluxDB ở dạng giả lập; và hành vi khóa nắp của virtual sensor. Tổng số chín mươi hai test, toàn bộ đều đạt.
...
Hướng phát triển tiếp theo gồm bảo mật kết nối MQTT bằng TLS kết hợp xác thực thiết bị chặt chẽ hơn, mở rộng thuật toán tối ưu lộ trình thu gom cho quy mô nhiều khu vực và nhiều gateway (hiện tại chỉ sắp xếp theo mức đầy, chưa tính khoảng cách địa lý hay thời gian di chuyển), nâng cao độ chính xác của mô hình dự báo thời điểm thùng đầy bằng cách bổ sung khoảng tin cậy 95\% dựa trên phần dư hồi quy và áp dụng cửa sổ trượt thích nghi thay vì cửa sổ cố định mười lăm phút, cùng việc nối thêm kênh thông báo qua email hoặc Telegram khi Alarm trên ThingsBoard chuyển sang mức nguy cấp.
